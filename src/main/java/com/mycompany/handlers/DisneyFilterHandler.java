...
public class DisneyFilterHandler implements SOAPHandler<SOAPMessageContext> {
    ...
    if(!outbound){
        ...
        try {
            ...
            NodeList genres = msg.getSOAPBody().getElementsByTagName("title");
            NodeList types = msg.getSOAPBody().getElementsByTagName("title");
            if(titles.item(0) != null){
                if(titles.item(0).getTextContent().toLowerCase().contains("disney")
                        || genres.item(0).getTextContent().toLowerCase().contains("disney")
                        || types.item(0).getTextContent().toLowerCase().contains("disney")){
                    return false;
                }
            }
